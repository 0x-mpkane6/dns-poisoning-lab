...
RESOLVER="${RESOLVER_IP:-10.80.0.53}"
...
echo "[+] E5-unbound probe: zone=${TARGET_ZONE}, rounds=${ROUNDS}, resolver=${RESOLVER}, profile=${PROFILE}"

for i in $(seq 1 "$ROUNDS")
do
    QNAME="u${i}$(monotonic_ns | cut -c1-4).example.net"

    START_NS="$(monotonic_ns)"
    if TRIGGER_OUT="$(dig @"$RESOLVER" "$QNAME" +tries=1 +time=2 +noall +answer +comments 2>&1)"; then
        TRIGGER_RC=0
    else
        TRIGGER_RC=$?
    fi
    ...
    TRIGGER_B64="$(printf '%s' "$TRIGGER_OUT" | base64 -w 0 2>/dev/null || printf '%s' "$TRIGGER_OUT" | base64)"
    printf "%s\t%s\t%s\t%s\t%s\n" "$i" "$QNAME" "$TRIGGER_RC" "$TRIGGER_STATUS" "$TRIGGER_B64" >> "$TRIGGER_RAW_FILE"

    START_NS="$(monotonic_ns)"
    if BANK_OUT="$(dig @"$RESOLVER" bank.com +tries=1 +time=2 +short 2>&1)"; then
        BANK_RC=0
    else
        BANK_RC=$?
    fi
