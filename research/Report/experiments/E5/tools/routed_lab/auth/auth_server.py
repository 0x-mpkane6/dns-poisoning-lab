...
from wire import build_dns_answer, build_tcp_frame, fragment_udp_payload
...
class AuthoritativeServer:
    ...
    def notify_attacker(self, *, qname: str, ipid: int, dport: int, dns_id: int, dns_len: int) -> None:
        payload = {
            "run_id": RUN_ID,
            "rep": REP,
            "workload": WORKLOAD,
        ...
        self.log.write("attacker_notify", qname=qname, ipid=ipid, dport=dport, dns_id=dns_id, dns_len=dns_len)
    ...
    def run(self) -> None:
        ...
        tcp.bind((AUTH_IP, 53))
        tcp.listen(64)
        self.log.write("auth_ready", udp_port=53, tcp_port=53, schedule_trials=len(self.schedule))
