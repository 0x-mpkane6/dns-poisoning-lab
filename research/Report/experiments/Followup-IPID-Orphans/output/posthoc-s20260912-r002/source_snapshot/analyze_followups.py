...
OUTPUT = HERE / "output/posthoc-s20260912-r002"
W = 2_000_000_000
SEED = 20260912
...
def pcap_fragments(path):
    ...
    return result,packet_count,{"capture_order_inversions":len(inversions),"max_capture_inversion_ms":max(inversions,default=0)/1e6}
...
def orphan_baseline():
    ...
    for index,run in enumerate(registered):
        ...
        run_rows.append({**base,"trials":50,"majority_query_rate":float(np.mean([r["majority_at_query"] for r in selected])),
            ...
            "clock_offset_range_ms":(max(offsets)-min(offsets))/1e6,**capture_audit})
