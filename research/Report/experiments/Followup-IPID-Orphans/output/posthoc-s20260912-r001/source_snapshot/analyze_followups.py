...
OUTPUT = HERE / "output/posthoc-s20260912-r001"
W = 2_000_000_000
SEED = 20260912
...
def pcap_fragments(path):
    ...
    result=[]; reader=RawPcapNgReader(str(path));previous=-1;packet_count=0
    try:
        for data,meta in reader:
            ...
            if timestamp<previous:raise ValueError("Unordered ingress PCAP")
...
def orphan_baseline():
    ...
    for index,run in enumerate(registered):
        ...
        fragments,raw_count=pcap_fragments(cell/"ips_outside.pcapng")
        ...
        for j,(start,end,trial) in enumerate(queries):
            ...
            entry={**base,"trial_id":trial["trial_id"],"query_start_wall_ns":start,"query_end_wall_ns":end,
                ...
                "any_during_query":any(s["orphan_any"] for s in candidates)}
            rows.append(entry)
        selected=rows[-50:]
        run_rows.append({**base,"trials":50,"majority_query_rate":float(np.mean([r["majority_at_query"] for r in selected])),
            ...
            "max_tail_ratio":max(s["orphan_ratio"] for _,s in tail_states),
            ...
            "clock_offset_range_ms":(max(offsets)-min(offsets))/1e6})
        audit.append({**base,"raw_packets":raw_count,"scoped_fragments":len(fragments),"query_states_independently_checked":50,"forged_logged":len(forged),"forged_observed":len(forged_seen)})
    ...
    write_json(folder/"verification.json",{"status":"PASS","runs":320,"trials":16000,"pcap_packets":total_raw,"scoped_fragments":total_fragments,
        ...
        "forged_matched_at_ingress":sum(r["matching_head_seen_at_ingress"] for r in forge_rows),
