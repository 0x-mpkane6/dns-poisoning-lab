...
The primary comparison is B5 (combined) versus B2 (volume-only).  Benign and
synthetic attack conditions in each pair share the exact same FRAG2 and query
timestamps, so occupancy is matched at every decision rather than fitted on the
reported data.
...
def simulate_condition_run(out_dir: Path, experiment_seed: int, schedule: SharedSchedule,
    ...
    with gzip.open(raw_path, "xt", encoding="utf-8", newline="\n") as raw_handle:
        ...
        for query_idx, (query_time, frag1_ipid) in enumerate(
            ...
            decisions = {
                variant: bool(R.r2_should_block(variant, True, total, entropy, unique_ratio))
...
def aggregate_test_results(test_runs: Sequence[RunData], thresholds: dict[str, Any],
    ...
    delta_by_attack_level: dict[tuple[str, int], dict[int, float]] = {}
    for attack in ATTACK_CONDITIONS:
        profile = CONDITIONS[attack]["profile"]
        ...
        for level in LEVELS:
            ...
            per_variant: dict[str, dict[str, list[float]]] = {
                variant: {"benign_trigger": [], "attack_alert": [], "youden_j": []}
                for variant in VARIANTS
            }
            feature_aps = {feature: [] for feature in FEATURES}
            threshold_values: dict[str, dict[str, list[float]]] = {
                feature: {"benign_trigger": [], "attack_alert": []} for feature in FEATURES
            }
            ...
            for pair_id in pair_ids:
                ...
                for feature in FEATURES:
                    ...
                    threshold_values[feature]["benign_trigger"].append(float(np.mean(
                        feature_value(benign, feature) >= threshold
                    )))
                    threshold_values[feature]["attack_alert"].append(float(np.mean(
            ...
            delta_result = metric_summary([deltas[pair_id] for pair_id in pair_ids], experiment_seed,
                                          attack, level, "delta_j_combined_minus_volume")
            ap_results = {
    ...
    for attack in ("attack_sweep_continuous", "attack_sweep_bursty"):
        ...
        common_pair_ids = sorted(set.intersection(*[
            set(delta_by_attack_level[(attack, level)]) for level in LEVELS
        ]))
        macro_values = [float(np.mean([
            delta_by_attack_level[(attack, level)][pair_id] for level in LEVELS
        ])) for pair_id in common_pair_ids]
        effect = metric_summary(macro_values, experiment_seed, attack, "macro_delta_j")
        ...
        macro_effects.append({
            "attack_condition": attack,
            "levels": list(LEVELS),
            "metric": "macro_average_delta_j_combined_minus_volume",
...
def write_summary_csv(path: Path, result: dict[str, Any]) -> None:
    with path.open("x", newline="", encoding="utf-8") as handle:
        ...
        for cell in result["test_cells"]:
            ...
            values = cell["delta_j_combined_minus_volume"]
            writer.writerow([
                *common, "delta_j_combined_minus_volume", "combined-minus-volume",
...
def main() -> int:
    ...
    for effect in analysis["macro_effects"]:
        value = effect["result"]
        print(f"  {effect['attack_condition']}: ΔJ={value['mean']:+.4f} "
              f"95%CI=[{value['ci95'][0]:+.4f},{value['ci95'][1]:+.4f}] "
              f"=> {effect['verdict']}")
    return 0
