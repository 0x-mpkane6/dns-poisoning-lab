...
def validate_results(results: dict[str, Any], raw: dict[tuple[str, str, int, int], dict[str, Any]],
    ...
    deltas: dict[tuple[str, int], dict[int, float]] = {}
    for attack in ATTACK_CONDITIONS:
        ...
        for level in LEVELS:
            ...
            per_variant = {variant: {"benign_trigger": [], "attack_alert": [], "youden_j": []}
                           for variant in VARIANTS}
            cell_deltas: dict[int, float] = {}
            ...
            expected_delta = bootstrap_mean([cell_deltas[pair_id] for pair_id in range(test_k)],
                                            derive_seed(experiment_seed, "bootstrap", attack, level,
                                                        "delta_j_combined_minus_volume"))
            try:
                compare_stats(cell["delta_j_combined_minus_volume"], expected_delta,
                              f"{attack}/{level}/delta", errors)
            except KeyError:
                errors.append(f"missing result delta {attack}/{level}")
    ...
    for attack in ("attack_sweep_continuous", "attack_sweep_bursty"):
        ...
        if entry is None:
            ...
            continue
        values = [float(np.mean([deltas[(attack, level)][pair_id] for level in LEVELS]))
                  for pair_id in range(test_k)]
        expected = bootstrap_mean(values, derive_seed(experiment_seed, "bootstrap", attack,
                                                      "macro_delta_j"))
        try:
